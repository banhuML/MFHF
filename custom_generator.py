...
def generator_td_simple_mopt(phase_gen='train'):
    ...
    while True:
        ...
        Ymaps2 = np.zeros((num_frames, 28, 28, 1)) + 0.01
        Yfixs2 = np.zeros((num_frames, 28, 28, 1)) + 0.01
        ...
        for i in range(0, video_b_s):
            ...
            Y = preprocess_maps(maps[start:min(start + num_frames * f_gap, len(images)):f_gap], shape_r_gaus,
                                shape_c_gaus)
            Y2 = preprocess_maps(maps[start:min(start + num_frames * f_gap, len(images)):f_gap], 28,
                                28)
            Y_fix = preprocess_fixmaps_png(fixs[start:min(start + num_frames, len(images)):f_gap], shape_r_gaus,
                                           shape_c_gaus)
            Y_fix2 = preprocess_fixmaps_png(fixs[start:min(start + num_frames, len(images)):f_gap], 28,
                                           28)
            ...
            Ymaps2[0:Y2.shape[0], :] = np.copy(Y2)
            Yfixs2[0:Y_fix2.shape[0], :] = np.copy(Y_fix2)
            ...
            Ymaps2[Y2.shape[0]:num_frames, :] = np.copy(Y2[-1, :, :])
            Yfixs2[Y_fix2.shape[0]:num_frames, :] = np.copy(Y_fix2[-1, :, :])
        yield Xims_ops, \
              [Ymaps, Ymaps, Yfixs, Ymaps,
               Ymaps, Ymaps, Yfixs, Ymaps,
               Ymaps, Ymaps, Yfixs, Ymaps,
               Ymaps, Ymaps, Yfixs, Ymaps,
               Ymaps2, Ymaps2, Yfixs2, Ymaps2
               ]  #
